...
async function ensureEnv(mode) {
  ...
  if (!(await access(envPath).then(() => true).catch(() => false))) {
    ...
    body = body
      ...
      .replace(/change-me-dashboard-password/g, secret("login-"))
    ...
    // COSMOG_TOKEN / EXEGGCUTE_API_KEY must stay real licenses — do not invent them.
    body = body.replace(/^COSMOG_TOKEN=change-me-cosmog-token$/m, "COSMOG_TOKEN=");
    body = body.replace(/^EXEGGCUTE_API_KEY=change-me-exeggcute-key$/m, "EXEGGCUTE_API_KEY=");
    const originIp = process.env.QADBAK_ORIGIN_IP?.trim();
    if (originIp) {
      body = body.replace(/SERVER_PUBLIC_IP=127\.0\.0\.1/, `SERVER_PUBLIC_IP=${originIp}`);
      if (!/^ROTOM_PUBLIC_WS=/m.test(body)) {
        body = `${body.replace(/\s*$/, "")}\nROTOM_PUBLIC_WS=ws://${originIp}:7070\n`;
      } else if (/^ROTOM_PUBLIC_WS=$/m.test(body) || /^# ROTOM_PUBLIC_WS=/m.test(body)) {
        body = upsertEnvLine(body.replace(/^# ROTOM_PUBLIC_WS=.*$/m, "ROTOM_PUBLIC_WS="), "ROTOM_PUBLIC_WS", `ws://${originIp}:7070`);
      }
    }
    await writeFile(envPath, body, "utf8");
  }
...
const ALLOWED_MODES = ["core", "mapping", "android", "ios", "phones", "full", "workers"];
...
const ANDROID_SERVICES = ["android-agent"];
const IOS_SERVICES = ["ios-agent"];

function wantsMapping(mode) {
  return mode !== "core";
}
function wantsAndroid(mode) {
  return mode === "android" || mode === "phones" || mode === "full";
}
function wantsIos(mode) {
  return mode === "ios" || mode === "phones" || mode === "full";
}
function wantsWorkers(mode) {
  return mode === "full" || mode === "workers";
}
...
async function startStack(mode) {
  ...
  if (!wantsMapping(mode)) return warnings;
  ...
  if (wantsAndroid(mode)) {
    try {
      await composeUp(ANDROID_SERVICES, ["android"]);
    } catch (e) {
      const detail = execDetail(e).slice(0, 500);
      warnings.push(`android-agent: ${detail}`);
      emit(`WARN: android-agent failed — continuing. ${detail}`);
    }
  }

  if (wantsIos(mode)) {
    try {
      await composeUp(IOS_SERVICES, ["ios"]);
    } catch (e) {
      const detail = execDetail(e).slice(0, 500);
      warnings.push(`ios-agent: ${detail}`);
      emit(`WARN: ios-agent failed — continuing. ${detail}`);
    }
  }

  if (wantsWorkers(mode)) {
    try {
...
export async function pogoStackInstall(domain, payloadJson) {
  ...
  const mode = String(payload.mode || "phones").trim();
  if (!ALLOWED_MODES.includes(mode)) {
    fail("Invalid mode — use core, mapping, android, ios, phones, workers, or full");
  }
  ...
  await ensureDocker();
  await ensureHostPrep(wantsWorkers(mode));
  ...
  const envBody = await readFile(path.join(POGO_DIR, ".env"), "utf8").catch(() => "");
  const dashboardUser = envBody.match(/^DASHBOARD_USER=(.*)$/m)?.[1]?.trim() || "admin";
  const dashboardPassword = envBody.match(/^DASHBOARD_PASSWORD=(.*)$/m)?.[1]?.trim() || "";
...
function buildPostInstall(mode, host, { arch, arm, warnings } = {}) {
  ...
  const lines = [
    ...
    "Add Pokémon GO accounts in the dashboard (login with DASHBOARD_USER / DASHBOARD_PASSWORD in .env).",
  ];
  if (wantsAndroid(mode) || wantsIos(mode)) {
    lines.push(
      `Open TCP 7070 from the phones to this VPS. Set ROTOM_PUBLIC_WS=ws://<public-ip>:7070 in .env.`,
    );
  }
  if (wantsAndroid(mode)) {
    lines.push(
      "Android: place cosmog.apk in integrations/pogo-stack/services/cosmog/apk/, set COSMOG_TOKEN, then bash scripts/provision-android.sh --device-id pixel-1",
    );
  }
  if (wantsIos(mode)) {
    lines.push(
      "iPhone: jailbreak + OpenSSH + Exeggcute/GC, set EXEGGCUTE_API_KEY, optional gc.deb in services/exeggcute/debs/, then bash scripts/provision-ios.sh --usb --device-id iphone-1",
    );
  }
  if (wantsWorkers(mode)) {
    lines.push(
