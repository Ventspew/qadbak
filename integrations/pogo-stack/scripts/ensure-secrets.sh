...
upsert DASHBOARD_PASSWORD "$(rand)"
...
if [[ -n "$PUB_IP" ]]; then
  ...
  upsert ROTOM_PUBLIC_WS "ws://${PUB_IP}:7070"
fi

# Cosmog / Exeggcute licenses must come from their Discords — never invent them
python3 - <<'PY'
import re
from pathlib import Path
text = Path(".env").read_text()
...
m = re.search(r"^EXEGGCUTE_API_KEY=(.*)$", text, re.M)
cur = (m.group(1).strip().strip('"').strip("'") if m else "")
if (not cur) or cur.startswith("change-me"):
    print("WARN: EXEGGCUTE_API_KEY still placeholder — set your real Exeggcute/GC key")
else:
    print("keep  EXEGGCUTE_API_KEY (already set)")
PY
...
print(f"Dashboard login:\n  {get('DASHBOARD_USER') or 'admin'} / {get('DASHBOARD_PASSWORD')}")
...
print(f"ROTOM_PUBLIC_WS:\n  {get('ROTOM_PUBLIC_WS')}")
print(f"COSMOG_TOKEN:\n  {get('COSMOG_TOKEN') or '(set real Cosmog license)'}")
print(f"EXEGGCUTE_API_KEY:\n  {get('EXEGGCUTE_API_KEY') or '(set real Exeggcute key)'}")
PY
fi
